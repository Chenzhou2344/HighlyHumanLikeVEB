...
from load_dataset_motion import *
...
class Host():
    ...
    def summarize_and_get_results(self) -> str:
        ...
        self.messages = [{"role": "system", "content": self.prompt['summer-system']},
                         {"role": "user", "content":[
                             {"type": "text", "text": message_},
                             {"type": "text", "text": 'The name of the AI medel is {}'.format(self.modelname)},
                             {"type": "text",
                              "text": 'These are the frames from the video generated by {}\n'.format(self.modelname)},
                             # {"type": "text", "text": f"{self.modelmessage}; Please carefully observe whether the actions required by the text prompt appear in the video\n"},
                             {"type": "text", "text": f"{self.modelmessage}\n"},
                             *map(lambda x: {"type": "image_url",
                                             "image_url": {"url": f'data:image/jpg;base64,{x}', "detail": "low"}},
...
def chat(config, prompt):
    ...
    file_handler = logging.FileHandler(config['log_path_motion'])
    ...
    with open("D:\\MemeAgent\\VideoEvalAgent\\data\\gpt\\motion_effects_multiagent_history_results.json", "w") as f:
        json.dump(history, f,indent=4)
    with open("D:\\MemeAgent\\VideoEvalAgent\\data\\gpt\\motion_effects_multiagent_updated_description_results.json", "w") as f:
        json.dump(des, f,indent=4)
    with open("D:\\MemeAgent\\VideoEvalAgent\\data\\gpt\\motion_effects_multiagent_ev_results.json", "w") as f:
